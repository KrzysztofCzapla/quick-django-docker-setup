...
class Urls:
    def __init__(self, use_jwt: bool, use_registration: bool, use_swagger: bool,
        ...
        self.important_lines = {
            "imports_last_line": 19,
            "patterns_last_line": 22
        }
    ...
    def add_paths(self):
        if self.use_jwt:
            self.insert_text_in_urls(self.important_lines["imports_last_line"],
        ...
        if self.use_swagger:
            self.insert_text_in_urls(self.important_lines["imports_last_line"], swagger_path)
            self.add_to_important_lines(5)

    def run(self):
        self.add_imports()

        if self.use_swagger: self.add_schema_view()
