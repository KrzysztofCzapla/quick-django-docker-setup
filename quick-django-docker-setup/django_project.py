...
class DjangoProject:
    ...
    def django_is_installed(self):
        try:
            subprocess.check_call([sys.executable, '-m', 'django', '--version'], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
            return True
    ...
    def install_django_in_venv(self):
        ...
        subprocess.check_call([pip_path, 'install', 'django'], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
    ...
    def execute_django_command_in_venv(self):
        ...
        subprocess.check_call([python_path, '-m', 'django', 'startproject', self.project_name])

        shutil.rmtree(self.env_path)

    def execute_django_command(self):
        subprocess.check_call([sys.executable, '-m', 'django', 'startproject', self.project_name])
